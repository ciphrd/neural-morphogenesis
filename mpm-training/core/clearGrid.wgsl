...
// (CH_J/CH_SHEAR/CH_PRESSURE) — this core has no display layer to feed.
// Kept as its own self-contained file rather than shared/imported (WGSL
// has no #include; see p2g.wgsl/g2p.wgsl's own header comments in the
// sandbox for why small duplication across files is this project's own
// deliberate convention, not an oversight).

const GRID_N: u32 = __GRID_N__u;
const NODE_COUNT: u32 = (GRID_N + 1u) * (GRID_N + 1u);
const CHANNELS: u32 = 3u; // must match p2g.wgsl/gridUpdate
