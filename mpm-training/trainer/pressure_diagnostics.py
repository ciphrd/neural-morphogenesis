...
def measure(snapshot, *, density=1., material_e=_DEFAULTS["materialE"], hardening=_DEFAULTS["materialHardening"], nu=_DEFAULTS["materialNu"]):
    ...
    g = np.linalg.det(fg)
    mass = (_DEFAULTS["particleMass"]/density)*rest[:, 15]*g
    ...
    elastic = np.sum(rest[:, 15]*g*energy_density*_DEFAULTS["particleVolume"]/density)
    ...
    return dict(samples=len(rest), elastic_energy=float(elastic), kinetic_energy=float(kinetic),
                ...
                grown_rest_area=float(np.sum(rest[:, 14]*g)),
                ...
                min_weight=float(rest[:, 15].min()), max_hardening=float(scale.max()))
