...
def check_gpu_pipelines() -> None:
    ...
    for architecture in ("persistent-environment", "cell-owned-projection"):
        core = MpmCore(device)
        environment = EnvironmentGPU(device, CHEM_CHANNELS, FIELD_N, FIELD_N, DECAY, DEPOSIT_RATE, architecture, grid_velocity=core.grid_vel, channel_profiles=CHEMICAL_CHANNEL_PROFILES)
        agents = AgentsGPU(device, core, environment, CHEM_CHANNELS, 128, MAX_ENV_WRITE, 16, SAMPLE_SPACING, FRICTION, 1.0, 0.5, 0.5, policy_architecture=STATEFUL_128_ARCHITECTURE, chemical_communication_architecture=architecture)
        ...
        sim = TrainingRollout(core, agents, environment, spawn_center=(0.5, 0.5), gravity=0.0, seed=7, mpm_enabled=False, initial_particle_count=1)
