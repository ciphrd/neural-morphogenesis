import channelConfigConfig from "../../../core/config.json";
const channelConfig = channelConfigConfig.chemistry;
...
function normalize(raw: PartialProfile, defaults: PartialProfile = {}): ChemicalChannelProfile {
  return {
    scale: raw.scale ?? defaults.scale ?? channelConfig.homogeneous.scale,
    resolutionScale: positive(raw.resolutionScale, defaults.resolutionScale ?? channelConfig.homogeneous.resolutionScale, "resolutionScale"),
    relaxationTime: positive(raw.relaxationTime, defaults.relaxationTime ?? channelConfig.homogeneous.relaxationTime, "relaxationTime"),
    fieldResponseTime: positive(raw.fieldResponseTime, defaults.fieldResponseTime ?? channelConfig.homogeneous.fieldResponseTime, "fieldResponseTime"),
    decayExponent: positive(raw.decayExponent, defaults.decayExponent ?? channelConfig.homogeneous.decayExponent, "decayExponent"),
    diffusionMultiplier: nonnegative(raw.diffusionMultiplier, defaults.diffusionMultiplier ?? channelConfig.homogeneous.diffusionMultiplier, "diffusionMultiplier"),
...
export function resolveChemicalChannelProfiles(
  ...
  const resolved = profiles ? profiles.map((profile) => normalize(profile)) : defaultChemicalChannelProfiles(channels);
