"""Rigid (rotation + translation) alignment search: finds the pose of
`points` that minimizes Chamfer distance to `target_points`, so the
reported distance reflects best-possible fit rather than being an
accident of whatever orientation/position the structure happens to have
grown in — nothing pins growth to the target's pose, and the target's own
orientation is an arbitrary artifact of however it was drawn, so fitness
(training and reporting alike) should score shape, not pose.
...
def best_fit_distance(points: np.ndarray, target_points: np.ndarray) -> dict:
    ...
    for i in range(NUM_RESTARTS):
        ...
        # centroid-matching translation is a good starting guess for any
        # given rotation, so the search only has to refine from there
        # rather than discover translation from scratch each restart
...
# Translation is solved analytically rather than searched: matching
# centroids is already a good fit for Chamfer distance, and the residual
# gap versus a fully-optimized translation is a second-order effect not
# worth chasing here. Rotation is a coarse grid search — a handful of
# direct Chamfer evaluations is far cheaper than Nelder-Mead's repeated
# numerical optimization (best_fit_distance's default cost is ~1800
# Chamfer evaluations per call; this is ~16).
TRAIN_NUM_ANGLES = 16
